...
namespace InfoBoxFactory
{
  ...
  public:
    ...
    c_InfoBox(int _type)
    {
      type = (t_InfoBox)_type;
    }

    c_InfoBox(unsigned _type)
    {
      type = (t_InfoBox)_type;
    }

    t_InfoBox min(t_InfoBox A)
    {
  ...
  struct InfoBoxMetaData {
    ...
    const c_InfoBox next, previous;
  };

  static const c_InfoBox::t_InfoBox NUM_TYPES = c_InfoBox::e_NUM_TYPES;
  ...
  GetName(const c_InfoBox &t)
  {
    assert(t.type < NUM_TYPES);

    return MetaData[t.type].name;
  }
   ...
   */
  static inline const TCHAR *
  GetCaption(const c_InfoBox &t)
  {
    assert(t.type < NUM_TYPES);

    return MetaData[t.type].caption;
  }
   ...
   */
  static inline const TCHAR *
  GetDescription(const c_InfoBox &t)
  {
    assert(t.type < NUM_TYPES);

    return MetaData[t.type].description;
  }

  static inline c_InfoBox::t_InfoBox
  GetNext(const c_InfoBox &t)
  {
    assert(t.type < NUM_TYPES);

    return MetaData[t.type].next.type;
  }

  static inline c_InfoBox::t_InfoBox
  GetPrevious(const c_InfoBox &t)
  {
    assert(t.type < NUM_TYPES);

    return MetaData[t.type].previous.type;
  }

  gcc_const
  InfoBoxContent* Create(const c_InfoBox &InfoBoxType);
};
